...
public class LoginController {
    ...
    public void login(@RequestBody LoginRequestBody body, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        if (!firstName.isEmpty()){
            // ** This HttpSession is the object that created when logging in
            // ** This is exactly different with the sessionFactory provided by Hibernate API,
            // ** who is used to create the object and interact with DB (ORM)
            HttpSession session = request.getSession();
            session.setAttribute("user_id", body.getUserId());
            // Here Tomcat will send the session ID back to browser, who will store it in cookie
            // this process is executed by tomcat automatically
            session.setMaxInactiveInterval(600);
